...
#import "DQValidationView.h"

#define DQRandomColor [UIColor colorWithRed:arc4random() % 100 / 100.0 green:arc4random() % 100 / 100.0 blue:arc4random() % 100 / 100.0 alpha: 0.5]

@interface DQValidationView ()
...
@implementation DQValidationView
...
-(instancetype)initWithFrame:(CGRect)frame  andCharCount:(NSInteger)charCount andLineCount:(NSInteger)lineCount{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        self.backgroundColor = DQRandomColor;
...
- (void)drawRect:(CGRect)rect {
    ...
    for(NSInteger i = 0; i < self.lineCount; i++) {
        // 设置线的颜色
        CGContextSetStrokeColorWithColor(context, DQRandomColor.CGColor);
        // 设置线的起点
